...
from app.analysis.models import AnalysisContext, ModuleStatus, ModuleType
...
def _context(transcript_result) -> AnalysisContext:
    """
    Sprint 4.5: every module's analyze() now takes an AnalysisContext,
    not a bare TranscriptProcessingResult. This test file's `transcript`
    fixture (and the handful of ad-hoc transcripts built inline below)
    are still plain TranscriptProcessingResult values, since most
    assertions read `transcript.metadata...` directly — this helper
    wraps one in the AnalysisContext a module actually receives, right
    at each call site, without touching the fixture's shape.
    """
    return AnalysisContext(transcript=transcript_result)
...
class TestDeterminism:
    ...
    async def test_running_twice_produces_identical_results(self, module_factory, transcript) -> None:
        ...
        first = await module.analyze(_context(transcript))
        second = await module.analyze(_context(transcript))
    ...
    async def test_does_not_mutate_the_transcript(self, transcript) -> None:
        ...
        for module in (FillerWordModule(), HesitationModule(), RepetitionModule(), SpeakingPaceModule()):
            await module.analyze(_context(transcript))
...
class TestFillerWordModule:
    async def test_total_matches_sprint_3_5s_default_dictionary_count(self, transcript) -> None:
        result = await FillerWordModule().analyze(_context(transcript))
    ...
    async def test_configurable_dictionary_changes_the_result(self, transcript) -> None:
        ...
        result = await module.analyze(_context(transcript))
    ...
    async def test_empty_transcript_returns_zero_not_a_failure(self) -> None:
        ...
        result = await FillerWordModule().analyze(_context(empty))
        assert result.status == ModuleStatus.OK
...
class TestHesitationModule:
    async def test_pause_count_matches_sprint_3_5(self, transcript) -> None:
        result = await HesitationModule().analyze(_context(transcript))
        assert result.status == ModuleStatus.OK
    ...
    async def test_long_pauses_use_the_stricter_threshold(self, transcript) -> None:
        result = await HesitationModule(long_pause_threshold_seconds=1.5).analyze(_context(transcript))
    ...
    async def test_empty_transcript_returns_zero_not_a_failure(self) -> None:
        ...
        result = await HesitationModule().analyze(_context(empty))
        assert result.status == ModuleStatus.OK
        assert result.metric.value == 0
...
class TestRepetitionModule:
    async def test_immediate_repetitions_match_sprint_3_5(self, transcript) -> None:
        result = await RepetitionModule().analyze(_context(transcript))
        assert result.status == ModuleStatus.OK
    ...
    async def test_empty_transcript_returns_zero_not_a_failure(self) -> None:
        ...
        result = await RepetitionModule().analyze(_context(empty))
        assert result.status == ModuleStatus.OK
        assert result.metric.value == 0
...
class TestSpeakingPaceModule:
    async def test_words_per_minute_from_metadata(self, transcript) -> None:
        result = await SpeakingPaceModule().analyze(_context(transcript))
        assert result.status == ModuleStatus.OK
    ...
    async def test_missing_duration_is_a_classified_failure_not_a_crash(self) -> None:
        ...
        result = await SpeakingPaceModule().analyze(_context(transcript_no_duration))
    ...
    async def test_no_pauses_gives_none_not_a_crash(self) -> None:
        ...
        result = await SpeakingPaceModule().analyze(_context(transcript_no_pauses))
